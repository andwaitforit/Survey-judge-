...
- [ ] **Personal data: respondent attributes.** The bias guardrail needs language/region/age band on *production* decisions, but CLAUDE.md says to ask before storing personal data beyond the answer text. For now attributes are accepted and dropped. Options: (a) keep dropping them; (b) store coarse bands only, with the same retention; (c) store them in a separate table with shorter retention. Which one?
- [ ] Retention purge is implemented (`DecisionLog.purgeExpired`) but nothing schedules it yet. Cron, pg_cron, or run on boot?
